...
function sendInvalidURLResponse(res) {
  return writeResponse(
    res,
    404,
    "url must be in the form of /fetch/{some_url_here}"
  );
}
...
if (cluster.isMaster) {
  ...
  http
    .createServer(async function (req, res) {
      ...
      if (req.url === "/health") {
        try {
          const response = await https.get("https://ipecho.net/plain");
          let realIP = "";
          response.on("data", (chunk) => {
            realIP += chunk;
          });
          response.on("end", () => {
            return writeResponse(res, 200, "hello");
          });
        } catch (error) {
          console.error(error);
          res.status(500).send("Unable to retrieve IP address");
        }
        return writeResponse(res, 200, "hi");
      }
